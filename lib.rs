...
mod astar_sns_contract {
    ...
    impl AstarSnsContract {
        ...
        pub fn release_post(
            ...
            self.release_post_fn(account_id, description, created_time, post_img_url);
        }
        ...
        pub fn send_message(
            ...
            self.send_message_fn(message, message_list_id, sender_id, created_time);
        }
        ...
        // トークンを送信する関数
        #[ink(message)]
        pub fn transfer(&mut self, from_id: AccountId, to_id: AccountId, amount: u128) {
            self.transfer_fn(from_id, to_id, amount);
        }

        // コントラクトから指定のアカウントへトークンを配布する関数
        #[ink(message)]
        pub fn distribute(&mut self, to_id: AccountId, amount: u128) {
            self.distribute_fn(to_id, amount);
        }

        // いいね数に応じてトークンを配布する関数
        #[ink(message)]
        pub fn distribute_refer_likes(&mut self, account_id: AccountId) {
            let total_likes = self.get_total_likes_fn(account_id);
            let asset = self.balance_of_fn(account_id);
            let calculated_amount = total_likes * 20;
            if asset < calculated_amount {
                self.distribute_fn(account_id, calculated_amount - asset);
            }
        }
    }
    ...
    //     }
        ...
        //     // プロフィール作成
        //     astar_sns_contract.create_profile(alice_account_id.clone());
        ...
        //     // プロフィール作成
        //     astar_sns_contract.create_profile(alice_account_id.clone());
        //     astar_sns_contract.create_profile(bob_account_id.clone());
        //     astar_sns_contract.create_profile(charlie_account_id.clone());
        ...
        //     // フォロー機能の確認
        //     astar_sns_contract.follow(alice_account_id, bob_account_id);
        //     debug_println!(
        //         "following_list: {:?}\nfollower_list: {:?}",
        ...
        //     // フォロー機能の確認(bob -> alice)
        //     astar_sns_contract.follow(bob_account_id, alice_account_id);
        //     // メッセージリストのidカウンターの確認
        //     debug_println!(
        //         "message_list_map_counter: {}",
        //         astar_sns_contract.message_list_map_counter
        ...
        //     astar_sns_contract.follow(alice_account_id, charlie_account_id);
        //     debug_println!(
        //         "following_list: {:?}\nfollower_list: {:?}",
        //         astar_sns_contract.get_following_list(alice_account_id),
        ...
        //     astar_sns_contract.distribute(alice_account_id, 100);
        //     astar_sns_contract.distribute(bob_account_id, 100);
        //     astar_sns_contract.distribute(charlie_account_id, 100);
